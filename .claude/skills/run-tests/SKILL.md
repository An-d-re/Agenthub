...
用 gstack browse 二进制驱动浏览器执行 `tests/cases/` 下的测试用例，保存 conversation.md + 截图供审阅。审阅发现的问题通过 `/debug-session` 诊断。
...
**⚠️ 所有浏览器操作必须通过 `$B` 二进制，禁止退化为 Python/Node 脚本。** 脚本固定逻辑无法应对 DOM 多分支，会导致空转。

...
export function TracePanel() {
  ...
  if (!sid || spans.length === 0) return (
    <div className="flex items-center justify-center h-full text-[13px] text-[#C7C7CC] text-center px-4">
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-4 py-3 shrink-0">
        <h3 className="text-[11px] font-semibold uppercase tracking-wider text-[#86868B]">调用追踪</h3>

        {/* Service filter */}
        <div className="flex gap-1 mt-2 flex-wrap">
          <button onClick={() => setServiceFilter(null)}
            className={cn("text-[10px] px-2 py-0.5 rounded-md transition-colors",
              !serviceFilter ? "bg-[#007AFF] text-white" : "bg-[#F5F5F7] text-[#86868B] hover:bg-[#E5E5E7]")}>
            全部
          </button>
          {services.map(svc => (
            <button key={svc} onClick={() => setServiceFilter(serviceFilter === svc ? null : svc)}
              className={cn("text-[10px] px-2 py-0.5 rounded-md transition-colors flex items-center gap-1",
                serviceFilter === svc ? "text-white" : "bg-[#F5F5F7] text-[#86868B] hover:bg-[#E5E5E7]")}
        ...
        {sortedTraces.length > 1 && (
          <select
            ...
            className="mt-2 w-full text-[11px] bg-[#F5F5F7] border-0 rounded-lg px-2 py-1.5 outline-none"
          >
        ...
        {filteredTraces.map(([tid, tSpans]) => {
          ...
          return (
            <div key={tid} className="border-t border-[#E5E5E7] first:border-t-0">
              <div className="px-4 py-1.5 flex justify-between bg-[#F9F9FB]">
                <span className="text-[10px] font-mono text-[#86868B]">{tid.slice(0, 8)}</span>
                <span className="text-[10px] text-[#C7C7CC]">{displaySpans.length} spans · {ms(traceTotal)}</span>
              </div>
                ...
                return (
                  <div key={s.span_id} className="px-4 py-1.5 hover:bg-[#F9F9FB] transition-colors"
                    ...
                    <div className="flex items-center gap-2 text-[11px]">
                      ...
                      <span className="flex-1 truncate font-mono">{s.operation_name}</span>
                      <span className={cn("tabular-nums", isError ? "text-[#FF3B30]" : "text-[#C7C7CC]")}>
                        {ms(s.duration_ms || 0)}
                      </span>
                    </div>
                    <div className="mt-1 h-1 bg-[#F5F5F7] rounded-full overflow-hidden">
                    ...
                    {s.tags && Object.keys(s.tags).length > 0 && (
                      <div className="flex gap-1 mt-1 flex-wrap">
                        {Object.entries(s.tags).slice(0, 4).map(([k, v]) => (
                          <span key={k} className="text-[9px] text-[#86868B] bg-[#F5F5F7] px-1 rounded">
